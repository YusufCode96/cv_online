    ...
    $data = $myFunction->select("tb_contact"); // Mengambil semua data 
    $table = "tb_contact"; // Inisialisasi database
    $form = "?page=contact";
    ...
    if(isset($_GET['hapus'])){ // Ketika ada uri segment ex : homepage.php?hapus&id = 1; kata disana adalah uri segment
        ...
        if($response['response'] == "positive"){ //Jika Berhasil
            ...
            echo "<script>window.location.href='dashboard.php?page=contact'</script>";
        }
    }

    if(isset($_POST['getCari'])){ // Ketika tombol cari di tekan
        $likeKey = "nama_user"; // Pencarian menurut / bisa disebut patokan yang ingin dicari
            ...
            foreach($data as $ds){ ?>
                <tr>
                    ...
                    <td colspan="2" class="text-center">
                        <a onclick="return confirm('Yakin Ingin Menghapus ?')" href="dashboard.php?page=contact&hapus&id=<?= $ds['id'] ?>" class="btn btn-danger" id="getDelete">Delete</a>
                    </td>
                </tr>
            <?php $no++; } 
        }else{ ?>
