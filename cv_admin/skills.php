    ...
    $data = $myFunction->select("tb_skill"); // Mengambil semua data 
    $table = "tb_skill"; // Inisialisasi database
    $form = "?page=skills";
    ...
    if(isset($_GET['hapus'])){ // Ketika ada uri segment ex : homepage.php?hapus&id = 1; kata disana adalah uri segment
        ...
        if($response['response'] == "positive"){ //Jika Berhasil
            ...
            echo "<script>window.location.href='dashboard.php?page=skills'</script>";
        }
    }

    if(isset($_POST['getCari'])){ // Ketika tombol cari di tekan
        $likeKey = "nama_skil"; // Pencarian menurut / bisa disebut patokan yang ingin dicari
...
<div class="panel-heading">Skills : Dashboard</div>
<div class="panel-body">
    <div class="row">
        <div class="col-sm-3">
            <a href="dashboard.php?page=addSkills" class="btn btn-primary">Tambah Data</a>
            <br>
            <br>
        </div>
            ...
            foreach($data as $ds){ ?>
                <tr>
                    ...
                    <td colspan="2" class="text-center">
                        <a href="dashboard.php?page=updateSkill&id=<?= $ds['id'] ?>" class="btn btn-default">Update</a>
                        <a onclick="return confirm('Yakin Ingin Menghapus ?')" href="dashboard.php?page=skills&hapus&id=<?= $ds['id'] ?>" class="btn btn-danger" id="getDelete">Delete</a>
                    </td>
                </tr>
            <?php $no++; } 
        }else{ ?>
